// Package db provides TimescaleDB connection for Layer 5
package db
...
// FetchCandles retrieves candles for a symbol
func (c *Client) FetchCandles(ctx context.Context, symbol string, limit int) ([]Candle, error) {
	...
	for rows.Next() {
		var candle Candle
		if err := rows.Scan(&candle.Time, &candle.Symbol, &candle.Open, &candle.High, &candle.Low, &candle.Close, &candle.Volume); err != nil {
			return nil, err
		}
...
// GetAvailableSymbols returns all symbols with data
