...
bot.command(
  'data',
  trackCommand('data', async (ctx) => {
    const symbol = ctx.message.text.replace('/data', '').trim().toUpperCase();
    ctx.reply(`📊 *Fetching Data Availability${symbol ? ` for ${symbol}` : ''}*...`);

    try {
      const url = symbol
        ? `http://backend-api:4000/api/v1/data-availability?symbol=${symbol}`
        : `http://backend-api:4000/api/v1/data-availability`;
      ...
      if (symbol) {
        if (data.message) {
          return ctx.reply(`❌ *${symbol}*: No data available.`);
        }
        const msg =
          `📊 *Data Availability: ${data.symbol}*\n\n` +
          `📅 *Start*: ${new Date(data.first_date).toLocaleDateString()}\n` +
          `📅 *End*: ${new Date(data.last_date).toLocaleDateString()}\n` +
          `📚 *Total Candles*: ${parseInt(data.total_candles).toLocaleString()}\n` +
          `🗓 *Trading Days*: ${data.trading_days}\n\n` +
          `_Base Granularity: 1-minute_`;
        return ctx.reply(msg, { parse_mode: 'Markdown' });
      }

      // Summary
      const summary = data.summary;
      const topSymbols = data.symbols
        .slice(0, 10)
        .map(
          (s) =>
            `• *${s.symbol}*: ${s.trading_days} days (${new Date(s.last_date).toLocaleDateString()})`
        )
        .join('\n');

      const msg =
        `🗄️ *System Data Watermark*\n\n` +
        `📈 *Total Symbols*: ${summary.total_symbols}\n` +
        `📅 *System Start*: ${new Date(summary.first_date).toLocaleDateString()}\n` +
        `📅 *System End*: ${new Date(summary.last_date).toLocaleDateString()}\n` +
        `📚 *Total Candles*: ${parseInt(summary.total_candles).toLocaleString()}\n` +
        `🗓 *Trading Days*: ${summary.trading_days}\n\n` +
        `📝 *Top Symbol Coverage*:\n${topSymbols}\n\n` +
        `_Use /data <symbol> for details (e.g. /data RELIANCE)_`;

      ctx.reply(msg, { parse_mode: 'Markdown' });
    } catch (err) {
      logger.error({ err }, 'Data Availability Fetch Error');
      ctx.reply('❌ Error fetching data availability from API.');
    }
  })
);
